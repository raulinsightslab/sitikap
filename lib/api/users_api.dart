...
class AuthenticationAPI {
  ...
  static Future<EditProfileModel> updateName({required String name}) async {
    final token = await PreferenceHandler.getToken();
    final url = Uri.parse(Endpoint.profile);
    ...
    if (response.statusCode == 200) {
      ...
      final error = json.decode(response.body);
      throw Exception(error["message"] ?? "Gagal mengubah nama");
    }
  }

  static Future<EditPotoProfile> updatePhoto({
    required File profilePhoto,
  }) async {
    final token = await PreferenceHandler.getToken();
    ...
    if (response.statusCode == 200) {
      return EditPotoProfile.fromJson(json.decode(response.body));
    } else {
      final error = json.decode(response.body);
